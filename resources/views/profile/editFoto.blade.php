        ...
        <form  method="POST" action="{{ route('dataImage.update', $dataImage->fotoID) }}"  enctype="multipart/form-data">
            @csrf
                    <div class="grid grid-cols-1 space-y-2">
                        ...
                        <input class="text-base p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-red-500" name="judulFoto" type="text" id="judulFoto" value="{{$dataImage->judulFoto}}" required >
                    </div>
                    <div class="grid grid-cols-1 space-y-2">
                        <label class="text-sm font-bold text-gray-500 tracking-wide">Deskripsi Image</label>
                        <textarea class="text-base p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-red-500" name="deskripsiFoto" type="text" value="{{$dataImage->deskripsiFoto}}" required>{{$dataImage->deskripsiFoto}}</textarea>
                    </div>
                    <div class="grid grid-cols-1 space-y-2">
...
<div class="flex flex-col gap-5">
    <div class="max-w-xs container bg-white rounded-xl shadow-lg transform transition duration-500 hover:scale-105 hover:shadow-2xl">
        <div class="px-3 w-full ">
            ...
            <img class="rounded-lg" src="{{ asset('image/' . $dataImage->lokasiFile) }}"  alt="{{$dataImage->judulFoto}}">
        </div>
...
<script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script><lottie-player src="https://lottie.host/346b73c4-54bd-42b4-96bb-b7721628c0a3/RCQjaZUlqC.json" background="##FFFFFF" speed="1" style="width: 300px; height: 300px" loop  autoplay direction="1" mode="normal"></lottie-player>
